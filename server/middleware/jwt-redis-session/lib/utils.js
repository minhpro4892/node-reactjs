...
module.exports = function (options) {
    ...
    _.extend(SessionUtils.prototype, {
        ...
        create: function (claims, callback) {
            ...
            if (!self.remember) {
                options.client.setex(options.keyspace + sid, options.maxAge, JSON.stringify(serializeSession(self)), function (error) {
                    self.id = sid;
                    if (!error) {
                        self.createWithUser(token, callback);
                    } else {
                        callback(error, token);
                    }
                });
            } else {
                options.client.set(options.keyspace + sid, JSON.stringify(serializeSession(self)), function (error) {
                    self.id = sid;
                    if (!error) {
                        self.createWithUser(token, callback);
                    } else {
        ...
        //create session depen userId
        createWithUser: function (token, callback) {
            var self = this;
            options.client.set('CC_USERID_SESS:' + self.user._id, self.id, function (error) {
                callback(error, token);
            });
        },
